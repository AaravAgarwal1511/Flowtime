...
- **Timezone model (`lib/tz.ts` + wall-clock helpers in `lib/time.ts`).** All wall-clock math — start-of-day, minute-of-day windows, weekdays, week keys, day enumeration — is computed in **`settings.timezone`** (an IANA zone on `UserSettings`), **never the server's own zone**. This matters because the server runs in UTC on Vercel while the calendar (FullCalendar, default `timeZone:"local"`) renders in the browser's zone; if the engine used server-local time, generated blocks would land at the wrong hour (e.g. an evening habit shoved to midnight) and appear to vanish on replan. `lib/tz.ts` is an `Intl`-based, DST-safe zoned-time engine (`zonedWallToUtc`, `wallParts`); the tz-dependent `lib/time.ts` helpers take an explicit `tz` arg, threaded through `SchedulerSettings.timezone` and `expandSeries(..., tz, ...)`. **`components/PlannerClient.tsx` self-heals**: on first load, if the browser's zone differs from `settings.timezone`, it PATCHes settings to the browser zone and replans once — so the engine and the view always agree.
...
`User` → `UserSettings` (work hours, buffer, weekly focus target, lock/plan horizons, `timezone`), `Task` (duration, min/max chunk, due, priority 1–4, status), `Habit` (frequency DAILY/WEEKDAYS/N_PER_WEEK, ideal window, kind), `Event` (the concrete block: type, `flexible`, `state`, `locked`, optional `sourceTaskId`/`sourceHabitId`), `TimeOff` (away ranges; `start` inclusive, `end` exclusive). The local DB lives at `prisma/dev.db`; `prisma/backups/` and `*.db` are gitignored.
